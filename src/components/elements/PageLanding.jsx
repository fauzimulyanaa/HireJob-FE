...
export default function PageHome() {
  ...
  console.log("ini worker", showWorker?.data?.result);
    ...
    <>
      ...
      <section>
        {showWorker?.data?.result?.map((items) => {
          return (
            <div className="dataProfile flex flex-col md:flex-row items-start px-8  w-5/6 mt-10 mx-auto bg-white p-5 mb-10" key={items.id_user}>
              <div className="user">
              ...
              <div className="nextProfile ms-auto my-auto mt-8">
                <button className="bg-violet-700 text-white rounded p-2 w-40">
                  <Link to={`/profile/${items.id_user}`}>Lihat Profile</Link>
                </button>
              </div>
            </div>
          );
